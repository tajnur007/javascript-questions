...
###### ৬৬. আমরা কোন কনস্ট্রাক্টর ব্যবহারের মাধ্যমে `Dog` ক্লাসটিকে এক্সটেন্ড করতে পারি?
...
একটি ডিরাইভ্ড (derived) ক্লাসে আপনি `super` কল করার আগে `this` কীওয়ার্ড অ্যাক্সেস করতে পারবেন না। যদি আপনি এটা করার চেষ্টা করেন, তবে এটি রেফারেন্স এরর দেয়: 1 এবং 4 নাম্বার কনস্ট্রাক্টরটি একটি রেফারেন্স এরর দেয়।

`super` কীওয়ার্ড দ্বারা আমরা প্যারেন্ট ক্লাসের কনস্ট্রাক্টরকে প্রদত্ত আর্গুমেন্টের মাধ্যমে কল করি। প্যারেন্ট ক্লাসের কনস্ট্রাক্টরটি `name` আর্গুমেন্ট গ্রহন করে, তাই আমরা `name` কে `super`-এ পাস করতে হবে।

`Labrador` ক্লাসটি দুটি আর্গুমেন্ট গ্রহন করে, `Dog` কে এক্সটেন্ড করা হয়েছে তাই `name` এবং অতিরিক্ত প্রপার্টি হিসেবে `Labrador` ক্লাসে `size` আর্গুমেন্ট গ্রহন করে। তাদের উভয়ই ঠিকমতো `Labrador` কনস্ট্রাক্টর ফাংশনে পাস করা দরকার, যা সঠিকভাবে কনস্ট্রাক্টর 2 এ করা হয়েছে।
...
`import` কীওয়ার্ড দ্বারা ইম্পোর্ট করা মডিউলগুলি _প্রিপার্স_ করা হয়। এর মানে, প্রথমে ইম্পোর্ট করা মডিউলগুলি  _রান_ করানো হয় এবং পরে যে ফাইলে কোড ইম্পোর্ট করা হয়েছে সেই ফাইলের কোড _রান_ করা হয়।

এটি `require()` এবং `import` এর মধ্যে একটি পার্থক্য! `require()` দ্বারা আপনি যখন কোডটি রান করবেন তখন ডিপেন্ডেন্সি গুলি অন-ডিমান্ড আকারে লোড করতে পারেন। আমরা যদি `import` এর পরিবর্তে `require` ব্যবহার করতাম, তবে `running index.js`, `running sum.js`, `3` কনসোলে লগ হতো।
...
প্রতিটি সিম্বলই পুরোপুরি ইউনিক। সিম্বলে পাস করা আর্গুমেন্টের উদ্দেশ্য হলো সিম্বলটির একটি বর্ণনা দেওয়া। সিম্বলের মান পাস করা আর্গুমেন্টের উপর নির্ভর করে না। সমতা পরীক্ষা করার জন্য আমরা সম্পূর্ণ নতুন দুটি সিম্বল তৈরি করেছি: প্রথমটি `Symbol('foo')`, এবং দ্বিতীয়টি `Symbol('foo')`। এই মান দুটি ইউনিক এবং এরা একে অপরের সমান নয়, এজন্য `Symbol('foo') === Symbol('foo')` কোডটি `false` রিটার্ন করে।
...
`padStart` মেথড দিয়ে আমরা একটি স্ট্রিং এর শুরুতে প্যাডিং যোগ করতে পারি। এই মেথডটিতে পাঠানো মানটি হলো স্ট্রিং এর _মোট_ দৈর্ঘ্য যা প্যাডিং সহ থাকবে। `"Lydia Hallie"` স্ট্রিং এর দৈর্ঘ্য হলো `12`। `name.padStart(13)` স্ট্রিং এর শুরুতে ১টি স্পেস যোগ করে, কারণ ১২ + ১ হলো ১৩।

যদি `padStart` মেথডে পাঠানো আর্গুমেন্টটি অ্যারের দৈর্ঘ্যের চেয়ে ছোট হয় তাহলে কোনো প্যাডিং যোগ হবে না।
...
- A: `"🥑💻"`
- B: `257548`
- C: তাদের কোড পয়েন্ট সম্মিলিত একটি স্ট্রিং
- D: ভূল
...
`+` অপারেটর ব্যবহার করে আপনি একটি স্ট্রিং এর সাথে অন্য স্ট্রিং যুক্ত করতে পারেন। এই ক্ষেত্রে আমরা স্ট্রিং `"🥑"` এর সাথে স্ট্রিং `"💻"` যুক্ত করছি, যার ফলাফল হলো `"🥑💻"`।
...
###### ৭১. যে মানগুলি কনসোলের পরে কমেন্ট করে রাখা হয়েছে সেগুলিকে আমরা কীভাবে লগ করতে পারি?
...
- A: `game.next("Yes").value` এবং `game.next().value`
- B: `game.next.value("Yes")` এবং `game.next.value()`
- C: `game.next().value` এবং `game.next("Yes").value`
- D: `game.next.value()` এবং `game.next.value("Yes")`
...
`String.raw` একটি স্ট্রিং রিটার্ন করে যেখানে এস্কেপ গুলি (`\n`, `\v`, `\t` ইত্যাদি) ইগনোর করা হয়! আপনি যখন নিচের স্ট্রিং এর মতো করে কোনো কিছু পেতে চান তখন ব্যাকস্ল্যাশগুলি সমস্যার কারণ হতে পারে:

`` const path = `C:\Documents\Projects\table.html` ``

যেটাকে কনসোল করলে আপনি এভাবে পাবেন:

`"C:DocumentsProjects able.html"`

`String.raw` এর মাধ্যমে আপনি খুব সহজেই এস্কেপ গুলিকে ইগনোর করতে পারবেন এবং এটা তখন নিচের মতো করে প্রিন্ট করবে:

`C:\Documents\Projects\table.html`

এই ক্ষেত্রে, `Hello\nworld` স্ট্রিং টি লগ হয়।
...
একটি এসিঙ্ক ফাংশন সর্বদা একটি প্রমিস রিটার্ন করে। `await` কে এখনও প্রমিস রিসলভ হওয়ার জন্য অপেক্ষা করতে হবে: যখন আমরা `getData()` কল করি তখন একটি পেন্ডিং প্রমিস রিটার্ন হয় যা `data` তে সেট হয়ে যায়।

যদি আমরা রিসলভ ভ্যালু `"I made it"` এক্সেস করতে চাই, তবে আমরা `data` এর উপর `.then()` মেথড ব্যবহার করতে পারতাম:

`data.then(res => console.log(res))`

এটি `"I made it!"` লগ করতো।
...
`.push()` মেথডটি নতুন অ্যারের _দৈর্ঘ্য বা লেন্থ (length)_ রিটার্ন করে! পূর্বে অ্যারেটিতে একটি মাত্র উপাদান ছিলো (স্ট্রিং `"banana"`) এবং তার দৈর্ঘ্য ছিল `1`। স্ট্রিং `"apple"` অ্যারেতে যোগ করার পর, অ্যারেটি দুটি উপাদান ধারণ করে এবং দৈর্ঘ্য হয় `2`। এটি `addToList` ফাংশন থেকে রিটার্ন করা হয়।

`push` মেথড মূল অ্যারেটিকে পরিবর্তন করে ফেলে। আপনি যদি ফাংশন থেকে অ্যারের দৈর্ঘ্যের পরিবর্তে অ্যারেটিকে রিটার্ন করতে চান, তবে আপনাকে `list`-এ `item` যোগ করার পরে এটি রিটার্ন করে দেওয়া উচিত।
...
`Object.freeze` একটি অবজেক্টের প্রোপার্টি যোগ করা, মুছে ফেলা, অথবা সংশোধন করাকে অসম্ভব করে (যদি প্রোপার্টির মান অন্য অবজেক্ট না হয়)।
...
পিওর ফাংশন হলো একটি ফাংশন যা একই আর্গুমেন্টের ক্ষেত্রে সবসময় একই ফলাফল রিটার্ন করে।

এখানে `sum` ফাংশনটি সবসময় একই ফলাফল রিটার্ন করে। আমরা ফাংশনটিতে `1` এবং `2` পাঠালে এটি কোনো ধরনের সাইড ইফেক্ট ছাড়াই সবসময় `3` রিটার্ন করবে। আমরা যদি `5` এবং `10` পাঠাই তাহলে এটি `15` রিটার্ন করবে এবং অন্যান্য আরো অনেক ক্ষেত্রেই এমনটা ঘটবে। এটিই হলো একটি পিওর ফাংশনের সংজ্ঞা।
...
`add` ফাংশনটি একটি _মেমোইজড_ ফাংশন। মেমোইজেশনের মাধ্যমে আমরা একটি ফাংশনের কার্যক্ষমতা বাড়াতে এর ফলাফলগুলি ক্যাশ করে রাখতে পারি। এই ক্ষেত্রে, আমরা `cache` নামে একটি অবজেক্ট তৈরি করেছি যা পূর্বে রিটার্ন করা মানগুলিকে সংরক্ষণ করে রাখে।

যদি আমরা একই আর্গুমেন্ট ব্যবহার করে আবার `addFunction` নামের ফাংশনটিকে কল করি, তবে এটি প্রথমে চেক করে যে এই মানটি ইতিমধ্যে তার ক্যাশে রয়েছে কিনা। যদি এই শর্ত মিলে যায় তবে ক্যাশ মানটি রিটার্ন হবে, যা এক্সিকিউশনের সময় সেভ করে। অথবা, যদি এটি ক্যাশে না থাকে তাহলে এটি মানটি ক্যালকুলেট করবে এবং পরবর্তীতে সেটি সংরক্ষণ করবে।

আমরা একই মান ব্যবহার করে `addFunction` ফাংশনটিকে তিনবার কল করেছি: প্রথমবার কল করার ক্ষেত্রে, ফাংশনটিতে যখন `num` এর মান `10` তখন মানটি এখনো ক্যাশ করা হয়নি। if-স্টেটমেন্টের শর্ত `num in cache` তখন `false` রিটার্ন করে এবং else ব্লকটি এক্ষেত্রে কাজ করবে: `Calculated! 20` লগ হয় এবং ফলাফলের মানটি ক্যাশ অবজেক্টে সংরক্ষণ হয়েছে। এখন `cache`-টিকে `{ 10: 20 }` দেখাবে।

দ্বিতীয়বার `cache` অবজেক্টটিতে `10` এর মান থাকায় এটি `10` এর জন্য সংরক্ষিত মানটি রিটার্ন করে। if-স্টেটমেন্টের শর্ত `num in cache` তখন `true` রিটার্ন করে এবং `'From cache! 20'` লগ হয়।

তৃতীয়বার আমরা ফাংশনটিতে `5 * 2` পাঠিয়ে দিই যা ইভালুয়েট হয়ে `10` হিসেবে রূপান্তরিত হয়। `cache` অবজেক্টটিতে `10` এর মান থাকায় এটি `10` এর জন্য সংরক্ষিত মানটি রিটার্ন করে। if-স্টেটমেন্টের শর্ত `num in cache` তখন `true` রিটার্ন করে এবং `'From cache! 20'` লগ হয়।
...
আমরা একটি _for-in_ লুপ ব্যবহার করে **enumerable** প্রপার্টিগুলি অর্থাৎ যেসব প্রপ্রার্টি গুলি গননা করা যায় সেগুলির উপর ইটারেট করতে পারি। একটি অ্যারেতে enumerable প্রপার্টিগুলি হলো অ্যারেতে অবস্থিত এলিমেন্টগুলির "কী (key)", যা সত্যিকার অর্থে তাদের ইনডেক্স। আপনি অ্যারেটিকে এভাবে বিবেচনা করতে পারেন:

`{0: "☕", 1: "💻", 2: "🍷", 3: "🍫"}`

যেখানে কী গুলি হলো enumerable প্রপার্টি। এজন্য প্রথমে `0` `1` `2` `3` লগ হয়।

একটি _for-of_ লুপ ব্যবহার করে আমরা **iterables** অর্থাৎ যার উপর ইটারেট করা যায় এমন **iterables** এর উপর ইটারেট করতে পারি। অ্যারে একটি iterable। যখন আমরা অ্যারেটির উপর ইটারেট করি, ভেরিয়েবল "item" হলো তার বর্তমান ইটারেশনের এলিমেন্ট বা উপাদান, এজন্য `"☕"` `"💻"` `"🍷"` `"🍫"` লগ হয়।
...
অ্যারেতে উপাদান হিসেবে যেকোনো মান ধারণ করতে পারে: নাম্বার, স্ট্রিং, অবজেক্ট, অন্যান্য অ্যারে, নাল, বুলিয়ান মান, আনডিফাইন্ড, এবং অন্যান্য এক্সপ্রেশন যেমন তারিখ, ফাংশন এমনকি ক্যালকুলেশনও।

উপাদানটি হবে রিটার্ন করা মানের সমান হবে। `1 + 2`, `1 * 2` এবং `1 / 2` ক্যালকুলেশন গুলো যথাক্রমে `3`, `2` এবং `0.5` রিটার্ন করে।
